...
document.addEventListener('DOMContentLoaded', function() {
    const API_URL = 'http://localhost:3005/bootcamps'; // Your db.json endpoint
    ...
    async function fetchBootcamps() {
      try {
        ...
        alert('Failed to load bootcamps. Please ensure db.json is properly configured.');
        bootcamps = []; // Fallback to empty array
      }
    }
    ...
    // Handle form submission - now saves to db.json
    async function handleFormSubmit(e) {
      ...
      try {
        ...
        if (!response.ok) throw new Error('Failed to add bootcamp');
